...
- [x] Написать тесты нормализованного сопоставления:
...
- [x] Запустить `uv run pytest tests/test_download_candidates.py -q`, подтвердить RED.
- [x] Реализовать сравнение casefold/NFKC, без расширения, пунктуация как
...
- [x] Объединить найденные по имени разрешённые записи с привязанными к hits
...
- [x] Создать request-scoped callback, принимающий только разрешённые ID
...
- [x] Описать Pydantic TokenEvent/DoneEvent и Attachment на границе API,
  не импортируя API DTO в core. DoneEvent сохраняет существующие поля и
  добавляет attachments с default_factory=list.
- [x] Проверить совпадения Premium, одинаковые названия фрагментов, запрещённый
...
- [x] Написать тест ошибок скачивания и совместимости потока:
...
- [x] Запустить `bun test tests/download.test.js tests/answer-stream.test.js`,
  подтвердить RED новых сценариев.
- [x] В загрузке добавить флаг false для новой пачки; snapshot значения хранить
...
- [x] Обновить тип Message и обработку done: attachments ?? []. Показывать
...
- [x] По клику fetch файла; 404 — «Файл больше недоступен», 429 — предложение
...
- [x] Проверить тестами 200/404/429, вредоносный URL, старый done, вложение без
...
- [x] Запустить `bun test && bun run check && bun run build`.
- [x] Коммит `feat: manage download permissions and show chat attachments`.
...
- [x] Обновить инструкции: 0001 → новая head через alembic upgrade head;
...
- [x] Документировать limiter одного процесса, общий NAT, доверенные прокси,
...
- [x] Дописать log-тест с caplog: успешное изменение имеет old/new и request_id,
...
- [x] На изолированной тестовой Postgres проверить upgrade с 0001 и сохранение
...
- [x] Сквозной сценарий через BFF с тестовыми файлами: закрытый источник знаний
...
- [x] Провести финальное ревью diff относительно spec, включая отсутствие
...
- [x] Коммит документации/финальных поправок `docs: document download tools and operational limits`.
...
Новые файлы: `backend/ragkb/core/answer_events.py`,
`backend/ragkb/services/download_candidates.py`,
`backend/tests/test_download_candidates.py`. Изменены
`backend/ragkb/services/downloads.py` (`tool_attachment`) и
`backend/ragkb/api/schemas/ask.py` (`TokenEvent`, `DoneEvent` с вложениями).

Решения по ходу:

- `core/answer_events.py` — только стандартная библиотека: `ToolCandidate`,
  `ToolResult`, `DownloadResolver`, `AnswerEvent` и сборка результата
  инструмента (`attachment_result`, `not_available_result`). Адрес скачивания
  строит сервер: `/api/documents/{id}/download`.
- `DownloadsService.tool_attachment` переиспользует `resolve` (та же проверка
  реестра, разрешения и безопасное открытие) и сразу закрывает дескриптор:
  инструменту нужны имя, размер и тип, а байты отдаёт отдельный маршрут.
- `prepare_candidates` собирает набор из названий, упомянутых в вопросе, и
  документов, к которым привязаны находки. Привязка идёт по пути *внутри*
  каталога корпуса, а не по имени файла: чужой путь с тем же именем корпусным
  документом не считается. Закрытые документы остаются в наборе с
  `download_allowed=false`, путей в наборе нет.
- Сопоставление названий: NFKC и casefold, пунктуация — разделители, служебные
  слова и расширения исключены. Точное название получает приоритет, при этом
  короткое название, целиком входящее в другое совпадение («MediaText» и
  «MediaText Premium»), не тянет за собой соседа. Если точных совпадений нет,
  работает пересечение содержательных слов, поэтому «HTML5» не выбирает
  Mobile-вариант молча, а показывает оба.
- Больше двадцати совпавших названий — кандидатов нет вовсе: случайная часть
  списка хуже пустого набора, а уточнение формулировки делает промпт (Task 5).
- `download_resolver` — обработчик на один вопрос: идентификатор вне набора
  получает `not_available`, даже если документ разрешён; разрешение всё равно
  перечитывается при вызове, поэтому отзыв после подготовки действует сразу.
- `core/ports.py` в этой задаче не менялся: порт инструментальной генерации
  (`stream_tool_answer`) появляется вместе с его реализацией в Task 5, а здесь
  ядро отдаёт только типы событий. Это осознанное отступление от списка файлов
  плана — добавлять в порт метод без реализации было бы мёртвым кодом.

Фактические результаты:

- RED: тесты падали на отсутствующем `ragkb.core.answer_events`.
- `uv run pytest tests/test_download_candidates.py -q` — 18 passed;
  с `tests/test_architecture.py` — 33 passed.
- Полный backend-набор — 268 passed, 1 deselected; ruff — те же 11 прежних
  замечаний.

Ограничения:

- Набор кандидатов — подсказка, а не разрешение: окончательное решение о
  выдаче принимает `DownloadsService.resolve` при вызове инструмента, и
  приложение файла без разрешения в реестре невозможно.
- Сопоставление названий не знает морфологии и синонимов: «мобильный» и
  «Mobile» — разные слова. Неоднозначность видна модели списком, а не
...
Новые файлы: `frontend/src/lib/download.js`,
`frontend/src/lib/components/chat/Attachments.svelte`,
`frontend/tests/download.test.js`. Изменены `lib/chat.svelte.js`,
`lib/components/chat/Message.svelte`, `routes/admin/documents/+page.svelte`,
`tests/answer-stream.test.js`.

Решения по ходу:

- `downloadAttachment` берёт адрес только из карточки и проверяет его
  регулярным выражением: собственный путь BFF `/api/documents/<id>/download`,
  идентификатор в пути обязан совпадать с `document_id` вложения. Чужой адрес
  (в том числе с другим идентификатором или с query) отклоняется **без**
  запроса — карточка не должна уметь увести браузер куда угодно.
- Отказы объясняются действием: 404 — «Файл больше недоступен», 429 —
  «Повторите через N с» из `Retry-After`, остальное — `detail` сервера.
  Тело отказа не сохраняется: это JSON, а не файл. `HEAD` перед `GET` не
  делается — ровно один запрос.
- Успешное тело собирается в `Blob` и сохраняется через объектный адрес,
  который освобождается после клика. Повторные клики блокируются состоянием
  карточки: пока идёт скачивание, кнопка недоступна.
- `Message.attachments` появляется из `done.attachments ?? []`: старый ответ
  без поля — не ошибка, просто нет карточек. Карточки рендерятся вне блока
  текста, поэтому видны и при пустом ответе; ошибка скачивания показывается
  рядом с файлом и текст ответа не затирает.
- В админке появился переключатель для новой пачки (по умолчанию выключен) и
  его снимок в `QueueItem`: изменение флага не меняет уже начатую очередь, а
  подтверждение замены прямо говорит, каким будет итоговое разрешение.
  Флаг уходит в загрузку через `URLSearchParams` вместе с `index=false`.
- В таблице — переключатель на каждый документ: `PATCH` по `document_id`,
  блокировка на время запроса, откат к прежнему значению при ошибке и
  сообщение о причине.

Фактические результаты:

- RED: `tests/download.test.js` сначала падал на отсутствующем модуле.
- `cd frontend && bun test` — 33 passed (7 скачивание, 2 потока с вложениями,
  13 прокси, 9 ограничитель, 2 прежних).
- `bun run check` — 0 ошибок, 0 предупреждений; `bun run build` собирается.
- Сквозная проверка через BFF на временном корпусе: загрузка с
  `download_allowed=true` — список отдаёт `document_id` и `download_allowed`;
  скачивание — 200 с `content-disposition`; `PATCH` в false — скачивание 404,
  обратно в true — снова 200; замена без флага — `download_allowed=false` при
  сохранённом `document_id`, скачивание 404.

Ограничения:

- Проверки, требующие браузера (две карточки в одном ответе, копирование,
  модалка источников, очистка чата, узкий экран, клавиатурные подписи),
  вручную не выполнялись: в этой среде нет браузера. Рендер карточек,
  доступные имена кнопок и переключателей проверены типами и сборкой, а не
  кликами.
- Карточка собирает файл в `Blob` целиком: при нынешнем лимите загрузки
  (20 МБ) это допустимо, но для больших файлов понадобится потоковая запись
  через File System Access API.
- Флаг в таблице меняется без подтверждения: контракт задачи намеренно открыт
  всем посетителям, отдельных ролей нет.
### Task 7 — выполнено

Изменены `README.md`, `frontend/README.md`, `AGENTS.md`,
`backend/tests/test_logging.py`; добавлены сквозной тест ответа с вложением и
подтверждение критериев приёмки.

**Документация.** В README описаны: две ревизии и порядок обновления прежней
базы (`stamp --purge 0001`, затем `upgrade head`; помечать сразу `0002`
нельзя — столбцов от этого не появится), новые маршруты, `attachments` в
`done`, раздел «Скачивание оригиналов» (управление флагом, независимость от
поиска и цитат, инструмент и требование tool calling, безопасная выдача,
замена и удаление), ограничение частоты с настройками и предупреждением про
`ADDRESS_HEADER`, раздел «Журнал». В ограничения добавлены открытость
управления флагом, невозможность отозвать уже скачанный файл, зависимость от
tool calling. `frontend/README.md` получил таблицу маршрутов BFF и настройки
ограничителя, `AGENTS.md` — состав `core/` и `services/`, контракт `/ask`,
две миграции и правила выдачи.

**Журнал (caplog).** `tests/test_logging.py`: успешное изменение разрешения
несёт `old/new`, `request_id` и идентификатор документа и не содержит имени
файла; отклонённое изменение не выдаётся за сохранённое; отказ выдачи
связывается с запросом по `request_id` и не превращается в событие
скачивания. Ротация журналов осталась прежней — отдельной таблицы и sink не
появилось.

**Сквозной тест.** `test_answer_attaches_the_original_and_revocation_closes_it`:
закрытый документ корпуса как источник ответа, разрешённый PDF как вложение,
`done.attachments` с адресом BFF, те же байты по маршруту backend, отзыв
разрешения закрывает ссылку.

**Postgres проверен на отдельной временной базе** (рабочая не затрагивалась,
временная удалена):

- `upgrade 0001 → head`: документы сохранены, идентификаторы уникальны,
  все прежние записи закрыты, чужая таблица `legacy_accounts` цела, ревизия
  `0002_document_downloads`; `NOT NULL` соблюдается;
- `downgrade 0001`: остаются только исходные столбцы, документы на месте;
  повторный `upgrade head` снова даёт уникальные идентификаторы и `false`;
- ветка `FOR UPDATE` (ранее не исполнялась нигде): `record` создаёт запись с
  запрошенным флагом, замена сохраняет идентификатор и сообщает фактическое
  прежнее значение; два одновременных снятия разрешения дают прежние значения
  `[False, True]`, то есть строки действительно сериализуются.

**Критерии приёмки спецификации** — чем подтверждены:

1. миграция на SQLite и Postgres — `tests/test_download_registry.py` и
   проверка на временной базе Postgres;
2. закрытый документ доступен поиску и цитатам, но не выдаче —
   `test_closed_document_stays_searchable_and_needs_no_reindex` и сквозной тест;
3. разрешённый PDF выдаётся по прямой просьбе, байты и заголовки совпадают —
   сквозной тест и живая проверка через BFF;
4. файл вне реестра, traversal, симлинки, пропавший файл и выдуманный
   идентификатор — `tests/test_downloads.py`;
5. флаг без переиндексации, отзыв закрывает старую ссылку, замена и повторная
   загрузка — те же тесты плюс сквозной;
6. настоящий tool call, фрагментированные аргументы, несколько вызовов,
   дедупликация, отказ, лимиты, обрыв и ответ без инструмента —
   `tests/test_tool_answers.py`, `tests/test_stream.py`;
7. кандидат по имени при DOCX в top-k и отсутствие случайного вложения при
   неоднозначности — `tests/test_download_candidates.py`;
8. карточки, переключатели, ошибка скачивания и старый `done` —
   `frontend/tests/download.test.js`, `answer-stream.test.js`, `bun run check`;
9. существующие и архитектурные проверки — backend 292 passed, frontend
   33 passed, `bun run check` без замечаний, `bun run build` собирается;
10. журнал — `tests/test_logging.py` и `tests/test_downloads.py`, события
    передачи проверены тестами прокси и живым BFF;
11. ограничение скачиваний — `tests/download-limiter.test.js`,
    `tests/download-proxy.test.js` и живая проверка адреса на dev-сервере.

**Финальное ревью diff.** В ветке 12 коммитов, 52 файла; среди изменений нет
файлов корпуса, `.env`, `data/` и настроек; `git diff --check` чист;
`data/docs` не тронут.

Ограничения, которые остались:

- Проверки, требующие браузера (клики по карточкам, копирование, модалка
  источников, очистка чата, узкий экран), вручную не выполнялись: браузера в
  этой среде нет.
- Настоящая tool-capable модель не запускалась: LLM-сервера рядом нет, все
  проверки идут на детерминированной скриптованной модели.
- Ограничение частоты — одного процесса frontend, квоты байтов нет.

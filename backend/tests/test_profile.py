...
def test_profile_returns_details(seeded) -> None:
    ...
    with _client(tmp_path, url) as client:
        _signin(client, "bob")
        res = client.get("/api/v1/auths/profile")
        assert res.status_code == 200
        body = res.json()
...
def test_change_password_ok(seeded) -> None:
    ...
    with _client(tmp_path, url) as client:
        ...
        bad = client.post(
            "/api/v1/auths/signin", json={"username": "bob", "password": "password1"}
        )
        assert bad.status_code == 401
        good = client.post(
            "/api/v1/auths/signin", json={"username": "bob", "password": "new-pass1"}
        )
...
def test_change_password_other_sessions_invalidated(seeded) -> None:
    ...
    with _client(tmp_path, url) as client:
        ...
        assert second.get("/api/v1/auths/me").status_code == 200
        ...
        assert client.get("/api/v1/auths/me").status_code == 200
        # …а вторая — погашена.
        assert second.get("/api/v1/auths/me").status_code == 401
        second.close()

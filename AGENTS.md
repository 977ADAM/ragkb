...
- `core/` — LangChain-ядро: `documents.py` (секции и чанки), `vectorstore.py`,
  `retrieval.py`, `pipeline.py` (LCEL-цепочка и ответ с инструментами),
  `tool_answers.py` (цикл модель → вызовы → результаты), `answer_events.py`
  (типы событий и кандидатов), конфиг, движок БД и ошибки.
...
- `services/` — сценарии через порты, без FastAPI, SQLAlchemy, LangChain,
  пайплайна и каталогов моделей: документы, выдача оригиналов (`downloads.py`),
  кандидаты на скачивание (`download_candidates.py`), ответ (`ask.py`).
...
- POST `/api/v1/ask` — независимый вопрос. NDJSON: только token и done
  (с источниками, предупреждениями, truncated и `attachments`). Вложения —
  внутренние события `attachment`: отдельными строками они не идут, а
  собираются в завершающее событие. Сервер не сохраняет переписку даже в
  памяти. Генерация обязательна: без `RAGKB_LLM_URL` запрос отклоняется с 503
  до открытия потока, экстрактивного ответа нет.
  `AskService.stream` — корутина: до открытия потока она выбирает модель,
  проверяет готовность генерации, ищет фрагменты (в отдельном потоке) и
  готовит кандидатов. Инструмент `get_download_link` требует модели с tool
  calling; без поддержки — явная ошибка, разбора ссылок в тексте нет.
...
- Миграция одна — `0001_corpus_documents`, и она создаёт реестр сразу в полном
  виде: `name`, `document_id`, `origin`, `uploaded_by`, `uploaded_at`, `size`,
  `sha256`, `download_allowed`, `index_enabled`. Промежуточных ревизий в
  каталоге нет; база собирается одной командой `alembic upgrade head`, а
  существующую базу либо пересоздают (документы лежат в `data/docs` и
  загружаются заново), либо помечают `alembic stamp --purge 0001_corpus_documents`
  — `--purge` обязателен, прежних ревизий в каталоге версий нет. Схему аккаунтов
  и переписки миграция не создаёт и не удаляет. `EXPECTED_REVISION` в
  `core/database.py` — эта же ревизия, её сверяет `tests/test_architecture.py`.
- У документа два независимых переключателя. `download_allowed` (по умолчанию
  выключен) разрешает выдать оригинал: `GET`/`HEAD /api/v1/documents/{id}/download`,
  файл открывается без прохода по симлинкам, содержимое сверяется с хэшем
  записи; поиск и цитаты от него не зависят. `index_enabled` (по умолчанию
  включён) решает, участвует ли документ в поиске: выключенный не индексируется,
  поэтому по нему нельзя ответить, но оригинал выдать можно. Оба флага меняются
  маршрутами `PATCH /api/v1/documents/{id}/download-permission` и
  `index-permission`; при замене файла идентификатор сохраняется, а флаги берутся
  из явных параметров загрузки. Смена `index_enabled` применяется следующей
  пересборкой индекса — поиск читает индекс, а не реестр.
- Ограничение частоты скачиваний живёт на BFF (`RAGKB_DOWNLOAD_*`), адрес
  берётся из соединения, а не из заголовков браузера; события передачи и IP
  пишет BFF, изменение флагов — backend. Скачанный файл отозвать нельзя.
...
- Корневой `.env` напрямую читается для `RAGKB_DATABASE_URL` и `POSTGRES_*`.

...
- Миграций две: `0001_corpus_documents` и `0002_document_downloads`
  (`document_id`, `download_allowed`). Схему аккаунтов и переписки они не
  создают, но и не удаляют: старые таблицы в уже существующих базах остаются
  нетронутыми. Такую базу помечают `alembic stamp --purge 0001_corpus_documents`
  (без `--purge` не выйдет — прежней ревизии нет в каталоге версий), а затем
  `alembic upgrade head`. Помечать сразу `0002` нельзя: столбцов от этого не
  появится. Не уничтожать существующие данные автоматически.
  `EXPECTED_REVISION` в `core/database.py` — текущая голова, её сверяет
  `tests/test_architecture.py`.
- Разрешение на скачивание оригинала — `download_allowed` в реестре, по
  умолчанию выключено. Меняется без переиндексации и только на выдачу файла:
  поиск и цитаты по закрытому документу работают. Прямой запрос идёт по
  `document_id` (`GET`/`HEAD /api/v1/documents/{id}/download`), файл открывается
  без прохода по симлинкам, а содержимое сверяется с хэшем записи. При замене
  файла идентификатор сохраняется, разрешение — нет.
- Ограничение частоты скачиваний живёт на BFF (`RAGKB_DOWNLOAD_*`), адрес
  ...
  пишет BFF, изменение разрешения — backend. Скачанный файл отозвать нельзя.
...
- Исторические планы не переписывать под новую архитектуру. Цепочка миграций
  намеренно сведена к начальной ревизии и одной добавочной — прежние восемь
  файлов восстанавливать не нужно.

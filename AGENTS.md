...
- POST `/api/v1/ask` — независимый вопрос. NDJSON: только token и done
  ...
  Правила про контекст описывают текст ответа и не отменяют просьбу прислать
  оригинал: замер на рабочей модели показал, что без этой оговорки вызов
  инструмента пропадал ровно тогда, когда контекст был. Поэтому просьба
  повторяется в самом вопросе порядком действий (`TOOL_STEPS` в
  `core/prompts.py`), а не только системными правилами.

...
def test_hash_is_stable_across_calls():
    from ragkb.embeddings import _stable_hash
...
def test_index_and_search_end_to_end_chroma():
    if not _chroma_available():
        print("      (chromadb не установлена — тест пропущен)")
        return
...
def test_chroma_incremental_update_and_delete():
    if not _chroma_available():
        return
    from ragkb.pipeline import RAGPipeline, build_index, remove_document, update_documents
...
def test_numpy_rejects_incremental_update():
    ...
    from ragkb.pipeline import build_index, update_documents
...
def test_invalid_collection_name_is_rejected_early():
    from ragkb.store import _validate_collection_name

/**
 * Разрешение на скачивание оригинала: PATCH через BFF.
 *
 * Ответ backend отдаётся как есть (JSON `detail` и статус), поэтому интерфейс
 * показывает ту же причину отказа. Метка запроса создаётся здесь и уходит в
 * backend: по ней строки журнала frontend и backend сопоставляются.
 */
...
export async function PATCH({ request, params }) {
	...
	try {
		upstream = await backend(
			`/documents/${encodeURIComponent(params.documentId ?? '')}/download-permission`,
			request,
			{
				method: 'PATCH',
